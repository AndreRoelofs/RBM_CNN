...
class RBM():
    ...
    def __init__(self, num_visible, num_hidden, k, learning_rate=1e-3, momentum_coefficient=0.5, weight_decay=1e-4, use_cuda=True):
        ...
        torch.nn.init.xavier_normal_(self.weights, 2.0)
    ...
    def contrastive_divergence(self, input_data, update_weights=True):
        ...
        positive_hidden_activations = (positive_hidden_probabilities >= 0.0).float()
        ...
        for step in range(self.k):
            ...
            hidden_activations = (hidden_probabilities >= 0.0).float()
    ...
    #
    #     p1 = torch.sum(torch.where(visible_probabilities > 0.5, 1, 0))
    #     p2 = torch.sum(torch.where(hidden_probabilities > 0.5, 1, 0))
    #
    #     for i in range(self.num_visible):
    #         for j in range(i, self.num_visible):
    #
