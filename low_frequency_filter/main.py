    ...
    'levels_info': [
        {
            ...
            'rbm_hidden_units': 300, 'encoder_learning_rate': 1e-3, 'n_training': 10
        },
    ]
}
...
def plot_energies(energies):
    ...
    #     # target_digit
    # ]:
    #     x = [e[0] for j, e in enumerate(energies) if int(e[1]) == i]
    #
    #     plt.plot(x, np.zeros_like(x) + i, markers[i], label="{}".format(i))
    #
    # plt.legend(numpoints=1, loc="lower left")
    # plt.show()

    # target_digit_indices = [n_train_data - (i + 1) for i, e in reversed(list(enumerate(energies))) if
    #                         int(e[1]) in target_digits]
    #
    ...
    #
    # one_hundred_test_history.append([one_hundred_test])
    # five_hundred_test_history.append([five_hundred_test])
    ...
    wdn.train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=2,
        shuffle=False,
        sampler=SubsetRandomSampler([random_tr_indice, similar_image_idx])
    )
...
def calculate_energies(node):
    ...
    for batch_idx, (data, target) in enumerate(wdn.train_loader):
        data = data.to(wdn.device)
        # distances = wdn.is_familiar(node, data, provide_value=True)[0]
        ...
        for i in range(len(distances)):
            d = distances[i]
            t = target[i]
            image_energies.append([d, t.numpy().astype(int), 5000 * batch_idx + i])
...
def create_node():
    ...
    wdn.train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=1,
        shuffle=False,
        sampler=SubsetRandomSampler([random_tr_indice])
        # sampler=SubsetRandomSampler(np.random.randint(0, n_train_data, 100))
    )
    for batch_idx, (data, target) in enumerate(wdn.train_loader):
        target_digit = target.numpy()[0]
        #
        # if target_digit in [5, 7, 8, 9]:
        #     target_digits = [5, 7, 8, 9]
        # else:
        #     target_digits = [0, 1, 2, 3, 4, 6]
        #
        ...
        #                                                  (wdn.levels[0]['rbm_visible_units']) *
        #                                                  wdn.levels[0]['encoder_channels'])
        # node.rbm.calculate_energy_threshold(flat_rbm_input)
        # image_energies = calculate_energies(node)
        # plot_energies(image_energies, real_run=False)
        ...
        #                                     lr=wdn.levels[node.level]['encoder_learning_rate'])
        flat_rbm_input = None
        for i in range(wdn.levels[0]['n_training']):
            # Encode the image
            rbm_input = node.encode(data)
            # Flatten input for RBM
        ...
        # print("Energy of og {}: {}".format(target_classes[target_digit], node.rbm.energy_threshold))
