...
def syntheticData():  # generate fake data
    ...
    trn_loader = utils.DataLoader(utils.TensorDataset(torch.from_numpy(X_train), torch.from_numpy(y_train)),
                                  batch_size=args.batch_size, shuffle=True, **kwargs)
    tst_loader = utils.DataLoader(utils.TensorDataset(torch.from_numpy(X_test), torch.from_numpy(y_test)),
...
def gradientCheck():
    ...
    inp = (Variable(torch.from_numpy(fake_output).double().cuda(), requires_grad=True),
           Variable(torch.from_numpy(fake_lbl).long().cuda(), requires_grad=False))
    ...
    inp = (Variable(torch.from_numpy(fake_input).float().cuda(),
                    requires_grad=True),)  # Variable(torch.from_numpy(fake_lbl).long().cuda(),requires_grad=False))
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(trn_loader):
        ...
        training_f1 += f1_score(target.data.cpu().numpy(), pred.cpu().numpy(), labels=np.arange(n_class).tolist(),
                                average='macro')
